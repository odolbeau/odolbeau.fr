...
- `page.html.twig` — the actual `<html>` document: head/metatags/JSON-LD, nav, footer, and third-party CDN includes (jQuery, Bootstrap). Everything else extends this and overrides Twig blocks (`head`, `stylesheets`, `header`, `content`, `javascripts`, ...) rather than duplicating markup.
...
Third-party scripts/styles are loaded straight from CDNs rather than vendored: jQuery/Bootstrap (`ajax.googleapis.com`/`cdn.jsdelivr.net`) in `page.html.twig`, Prism (`cdn.jsdelivr.net`) in `blog/page.html.twig`, SpeakerDeck (`speakerdeck.com`) in `talks/page.html.twig`, plus occasional inline GitHub Gist embeds (`gist.github.com` / `github.githubassets.com`) in individual post Markdown. The production server enforces a Content-Security-Policy (not part of this repo — set at the nginx/reverse-proxy level) allow-listing exactly these origins. **Adding a reference to any new external origin in a layout or post requires a matching update to that server-side CSP header**, or the resource will be blocked/reported in browsers.
